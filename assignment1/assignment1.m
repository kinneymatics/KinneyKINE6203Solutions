...
ver        %<- command goes here (to left of %)

% 5. Write some commands that would return a list of all of the available 
% functions in the image processing toolbox? 

% you were right, this was tough to find... The only way I found
% it was bc the pdf file for image processing toolbox documentation was 
% named 'images'
help images 
...
figure
plot(y,x)        %<- command goes here
hold on
plot(x,y) % wooooow, so pretty
...
%
% Answer: anova2() performs a two-way ANOVA. I would use this to determine 
% if there is there a significant difference between mean fixation 
% frequency & duration as a function of kick type & outcome (Piras &
% Vickers, 2011)
...
%
% Answer: for loops - loop through the code x times (keeping track of each
% iteration, typically with a counter or index variable); while loops -
...
%
% Answer: Not a Number (NaN) is returned when you receive an indeterminate
% answer (i.e., 0/0, inf/inf, -inf - -inf, etc.) 
%
...
xpbombs % shoutout reddit for the answer
% https://www.reddit.com/r/matlab/comments/q2rlq/did_you_know_you_can_play_minesweeper_in_matlab/
...
decimalNumber = pi;
int32Number = uint32(decimalNumber);
whos
...
positivesOriginalVec = sum(originalVec > 0);

% - get the absolute value of each element

absoluteOriginalVec = abs(originalVec);
...
base1 = 10; % assigns the length of bottom of trapezoid
base2 = 7; % assigns the length of top of trapezoid
height = 6.66; % assigns the height of trapezoid
trapArea = ((base1 + base2)/2) * height % add bases, divide by 2, 
...
totalRevenue = input("How much was your gross income this year? ");
totalCosts = input("How much did you spend this year? ");
...
disp(input("Type whatever you'd like! ", "s"))
